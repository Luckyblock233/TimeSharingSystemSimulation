        ...
        if(tip == 1) {
            ...
            for(int i = 0; i < memoryList.length; i++){
                ...
                if(memoryList[i] == 9999){
                    ...
                    g2.drawRect(0, start, 170, rev*Infomation.osSize);
                    g2.fillRect(0, start, 170, rev*Infomation.osSize);   // 填充矩形颜色
                    Font font = new Font("SimSun", Font.BOLD, 12);
                    g2.setFont(font);
                    g2.setColor(Color.BLACK);
                    g2.drawString("OS", 80, (start+rev*Infomation.osSize)/2 +5); // 绘制文字
                    start += Infomation.osSize * rev;
                    i += Infomation.osSize - 1;
